...
from neural_sph.rollout import infer
...
def train_or_infer(args: Namespace):
    ...
    if args.config.mode == "infer" or args.config.mode == "all":
        ...
        config_dict = vars(args.config)
        params_redist = {
            "variant_p": config_dict.get("redist_variant_p", "standard"),
            "variant_visc": config_dict.get("redist_variant_visc", "standard"),
            "loops": config_dict.get("redist_loops", 1),
            "acc": config_dict.get("redist_acc", 0.015),
            "rho_threshold": config_dict.get("redist_rho_threshold", 0.98),  # disabl 0
            "reinit": config_dict.get("redist_reinit", 0.0),  # disable with 0
            "visc": config_dict.get("redist_visc", 0.0),
            "limiter": config_dict.get("redist_limiter", 3.0),
        }
        import pprint

        pprint.pprint(params_redist)
        metrics = infer(
            model,
            case,
        ...
        print(f"Metrics on {args.config.model_dir}, {split} split:")
        metrics_ave = averaged_metrics(metrics)
        print(metrics_ave)
        with open(osp.join(args.config.rollout_dir, "metr_ave.json"), "w") as f:
