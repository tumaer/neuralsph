...
def cli_arguments() -> Dict:
    ...
    parser.add_argument(
        ...
        required=False,
        type=str,
        choices=["None", "stay", "adv", "standard", "riemann"],
        help="Variant of the pressure term in the particle redistribution.",
    )
    parser.add_argument(
        "-rvv",
        "--redist_variant_visc",
        required=False,
        type=str,
        help="Type of viscous term in the redistribution.",
    )
    parser.add_argument(
        "-rl",
        "--redist_loops",
        required=False,
        type=int,
        help="Number of iterations of the density relaxation.",
    )
    parser.add_argument(
        "-ra",
        "--redist_acc",
        required=False,
        type=float,
        help="Acceleration factor for the redistribution.",
    )
    parser.add_argument(
        "-rrt",
        "--redist_rho_threshold",
        required=False,
        type=float,
        help="Threshold for rho=np.where(rho<threshold, 1, rho).",
    )
    parser.add_argument(
        "--redist_reinit",
        required=False,
        type=float,
        help="Threshold for reinitialization, see Zhang (2017).",
    )
    parser.add_argument(
        "--redist_visc",
        required=False,
        type=float,
        help="Viscosity coefficient during the redistribution.",
    )
    parser.add_argument(
        "--redist_limiter",
        required=False,
        type=float,
        help="Limiter in Riemann SPH.",
    )
